...
extension MS2109VideoChipset: VideoChipsetHIDRegisters {
    ...
    // MARK: - MS2109 HID Report Format
    /// MS2109 uses standard format without 0x01 prefix
    ...
    // MARK: - MS2109 UInt16 Response Parsing
    /// MS2109 returns only one byte per read, so we need to read the next address for the low byte
