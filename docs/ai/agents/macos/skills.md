...
- If the intended app or panel is not visible, do not guess its Dock or window location; use a macro, keyboard path, or re-capture first.
...
- On macOS, treat `<CMD>` as Command and prefer Command-based shortcuts when they are the standard path.
